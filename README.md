...
守护循环的触发点**对齐墙钟**：`keepAlive(60)` 的每轮都落在每分钟 `:00`，
不会因任务执行耗时越漂越晚、整分钟被跳过；跑超时也不补偿式连发。

**时间窗去重**：5 段（分钟级）表达式的 `isDue()` 在整分钟内逐秒为真。
`keepAlive` 检测到秒级任务后自动提频到每秒轮询，此时若无保护，分钟级任务会被打成一分钟 60 次。
因此守护模式内置**进程内时间窗去重**：分钟级任务一分钟只派发一次、秒级任务一秒一次。
手动调用 `run()` 不受此限制（保持显式重跑的旧语义）。

多个进程/节点同时跑同一份任务表、又不想做 Leader 选举时，可开**跨进程派发去重**：

```php
$scheduler->useTickGuard('/shared/nfs-dir/ks-ticks');
// 同一 (任务, 时间窗) 只有一个进程认领成功，其余静默跳过
```
...
- **异常隔离**：单个任务抛错默认不中断其余任务，统一进入 `RunReport` 与 `onError()`；
  需要「遇错即停」时调用 `stopOnError()`——它也会**先完整落账**（记入失败、跑完 afterRun 钩子）
  再抛异常，拿到的永远是全量报告而非半截现场。
...
- **防重叠**：`withoutOverlapping()` 配合 `MutexInterface` 保证同一任务同一时刻只有一个执行单元在跑。
  默认的 FileMutex 基于 flock——进程退出时操作系统自动释放，**无 TTL 概念**，
  `overlapTtl()` 仅对分布式锁实现（如 Redis SETNX）有意义。
  锁键中的相对路径与 `../` 段一律拒绝直通（防锁文件穿越写入），只有干净的绝对路径按原样使用。
...
**Q：匿名任务（`task()`）的名字跨重启稳定吗？** 稳定。指纹取闭包的「定义位置（文件+行+作用域类）」，
不再是进程内的 `spl_object_hash`——去重表、锁文件、监控面板因此可以跨重启关联同一任务。
循环里同出处生成的多个闭包会自动追加 `#2`、`#3` 序号区分。
